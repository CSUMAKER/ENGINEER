...
float vision_get_speed_2020(float x_now,float x_last);

float vision_target_x_2020,vision_target_y_2020,temp_vision_target_x_2020,temp_vision_target_y_2020;
float vision_tar_base_x_2020,vision_tar_base_y_2020,vision_tar_kalman1_x_2020,vision_tar_kalman1_y_2020;
float vision_tar_kalman2_x_2020,vision_tar_kalman2_y_2020,vision_tar_kalman3_x_2020,vision_tar_kalman3_y_2020;

u8 vision_kalman_flag1,vision_kalman_flag2,vision_kalman_flag3;
float kalman_q1_2020,kalman_f1_2020,kalman_q2_2020,kalman_f2_2020,kalman_q3_2020,kalman_f3_2020;		
													//1是对base的kalman滤波，2是对增加速度模型后的滤波，3是增加加速度模型再滤波
...
struct HolderKalmanType vision_yaw_kalman1_2020,vision_yaw_kalman2_2020,vision_yaw_kalman3_2020;

void vision_data_deal(float offset_x,float offset_y,float offset_z)
{
	u8 init_flag = 1;			//防止保存第一个数据出现阶跃。。。。其实影响不大
	volatile static float last_tar_x,last_speed_x;
	temp_vision_target_x_2020 = holder_old_yaw - (float)(offset_x * camera_deal_kx + camera_deal_kb);

	if(init_flag)
	{
		init_flag = 0;
		last_tar_x = temp_vision_target_x_2020;
	}
	
	holder_kalman_init(kalman_q1_2020,kalman_f1_2020,&vision_yaw_kalman1_2020);
	vision_tar_kalman1_x_2020 = holder_KalmanFilter_cal(temp_vision_target_x_2020,&vision_yaw_kalman1_2020);
	if(!vision_kalman_flag1)					//标志位为0就不滤波了
		vision_tar_kalman1_x_2020 = temp_vision_target_x_2020;

//	temp_vision_target_x_2020 = vision_tar_kalman1_x_2020 + vision_get_speed_2020();			//增加速度处理
	holder_kalman_init(kalman_q2_2020,kalman_f2_2020,&vision_yaw_kalman2_2020);
	vision_tar_kalman2_x_2020 = holder_KalmanFilter_cal(temp_vision_target_x_2020,&vision_yaw_kalman2_2020);
	if(!vision_kalman_flag1)					//标志位为0就不滤波了
		vision_tar_kalman2_x_2020 = temp_vision_target_x_2020;

	temp_vision_target_x_2020 = vision_tar_kalman2_x_2020;			//增加加速度处理
	holder_kalman_init(kalman_q3_2020,kalman_f3_2020,&vision_yaw_kalman3_2020);
	vision_tar_kalman3_x_2020 = holder_KalmanFilter_cal(temp_vision_target_x_2020,&vision_yaw_kalman3_2020);
	if(!vision_kalman_flag1)					//标志位为0就不滤波了
		vision_tar_kalman3_x_2020 = temp_vision_target_x_2020;
