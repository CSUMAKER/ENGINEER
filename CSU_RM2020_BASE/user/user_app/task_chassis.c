...
void	task_chassis_mode_deal(void* param)
{
	...
	while(1)
	{
		...
		if(data != NULL)
		{
			...
			{
				case RP_S_UP:
					chassis_mode = SOLDIER_CATWALK;//SOLDIER_CATWALK//SOLDIER_SCOPE
					break;
				case RP_S_MID:
					chassis_mode = SOLDIER_FOLLOW;
					break;
...
void	task_chassis_control(void* param)
{
	...
	while(1)
	{
		...
		if(tick_count == 4)
		{
			...
			for(i=0;i<4;i++)           
			{
				chassis_V_T[i]=SOLDIER_RPC_ZERO(chassis_V_T[i],30); 
...
void chassis_follow_deal(chassis_ctrl_p data)
{
	...
	float follow_k,angle_abs = abs_f(angle);
...
//	if(angle_abs > 25)
//		follow_k = 20;
//	else if(angle_abs > 15)
//		follow_k = 15;
//	else if(angle_abs > 10)
//		follow_k = 8;
//	else
//		follow_k = 3;

//	clockwise = SOLDIER_LIMIT(follow_k * angle,-800,800);

	chassis_move_speed(front,right,clockwise);
}

#define SCOPE_CLOCKWISE  		500
...
#define CAT_CLOCKWISE_ANGLE  		30
#define CAT_CLOCKWISE_SPEED  		300
void chassis_catwalk_deal(chassis_ctrl_p data)
{
	static S16 catwalk_direction=1,differ_cat_angle;
	s32 front,right,clockwise,front_temp,right_temp;
	float angle_differ = -Dir_angle;
	...
	if(abs(differ_cat_angle)<3){
		catwalk_direction*=-1;
	}
	differ_cat_angle = catwalk_direction*CAT_CLOCKWISE_ANGLE-(angle_differ);
	clockwise=-catwalk_direction*CAT_CLOCKWISE_SPEED;
