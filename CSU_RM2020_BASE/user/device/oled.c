...
/*
	...
	OLED_WR_Byte(0x81, OLED_CMD, 1);//对比度设置
	OLED_WR_Byte(0xFF, OLED_CMD, 1); //1~255;默认0X7F (亮度设置,越大越亮)
    ...
    OLED_WR_Byte(0xF1, OLED_CMD, 1); //Set Pre-Charge as 15 Clocks & Discharge as 1 Clock 0xf1
    ...
    OLED_WR_Byte(0x02, OLED_CMD, 1); //---set low column address
    OLED_WR_Byte(0x8D, OLED_CMD, 1); //--set Charge Pump enable/disable0x8D
