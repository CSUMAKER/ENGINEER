...
bool is_ui_page_main = true;
bool is_supercup_work = false;
//***********************************双向链表部分*******************************************
    ...
    if (Get_KeyFifo(&KeyStatus))
    {
        ...
        case Key_SuperCup:
            if (KeyStatus.status == 0)
            {
                if(is_ui_page_main)
                {
                    is_supercup_work = !is_supercup_work;
                }
            }
        default:
            break;
...
void UI_show(P_Page_TypeDef p_page)
{
    ...
    while (1)
    {
        ...
        if (p_page->is_mainmenu == 1)
        {
            is_ui_page_main = true;
            if(is_supercup_work)
            {
                OLED_printf(103, 0, "scw");
            }
            else
            {
                OLED_printf(103, 0, "   ");
...
void data_save()
{
    ...
    Flash_Write32BitDatas(ADDR_FLASH_SECTOR_10, data_num, (int32_t*)data_save_buff);
...
void data_read()
{
    ...
    Flash_Read32BitDatas(ADDR_FLASH_SECTOR_10, FLASH_BUFF_LENTH, (int32_t*)data_read_buff);
