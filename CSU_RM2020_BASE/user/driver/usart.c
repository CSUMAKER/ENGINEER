...
void    uasrt4_init(u32 baud)
{
	USART_InitTypeDef	USART_InitStructure;
	NVIC_InitTypeDef	NVIC_InitStructure;
	
	#if defined MAIN_CONTROL_2020solider
	...
	#if defined MAIN_CONTROL_2020sentry
	GPIO_InitTypeDef	GPIO_InitStructure;
...
void	serial_init(U32	num, U32 baudrate)
{
	...
	if(num == 2)
	{
		#if defined(MAIN_CONTROL_2019)||defined(MAIN_CONTROL_2020sentry)
...
{
	u8 com_data;
	if(UART4->SR & USART_SR_ORE) //ORE中断（溢出中断）
    {
        com_data = UART4->DR;
    }
    //接收中断
